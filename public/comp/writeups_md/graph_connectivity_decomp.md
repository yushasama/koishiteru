...
You have graphs, you care about who's connected to who, where cycles live, and how to compress structure so DP / logic becomes easy.
...
* **Cycle (undirected)**
  A path that starts and ends at the same vertex, length ≥ 3, no repeated edges.
...
* **2-SAT**
  Boolean formula in CNF where each clause has exactly two literals, like
...
* **2-SAT**
  ...
  * Clauses: up to around \(2 \cdot 10^5\).
  * Implication graph: `2n` nodes, `2m` edges → `O(n + m)` SCC.

Constraint-based diagnosis:

* Undirected + "connected?", "how many components?", "tree or not" → CC / cycles.
* Directed + "mutually reachable", "infinite walk", "compress strongly connected parts" → SCC + condensation.
* "Each item has 2 options" + many pairwise "if/then / at least one / not both" constraints → 2-SAT.
...
* **SCC**
  ...
  * Define `u ~ v` if `u` and `v` can both reach each other. This is an equivalence relation → SCCs are equivalence classes.
...
* **2-SAT**
  ...
  * Build implication graph. If `x` and `¬x` end up in same SCC, you're forced to set `x` both true and false → impossible.
...
| DSU for CC             | Offline / dynamic edges              | `comp[v]` per vertex     | DSU             | `O(m α(n))` | Amortized near-constant.  |
...
* For each unvisited vertex, traverse its component:
  ...
  * If an edge `(v, to)` is seen where `to` is already visited and `to != parent[v]`, a cycle exists.
...
|------------|---------|-----------|--------|
| `(a ∨ b)` | At least one is true | `(!a → b) ∧ (!b → a)` | `add_or(x, 1, y, 1)` |
| `(!a ∨ !b)` | Not both true | `(a → !b) ∧ (b → !a)` | `add_or(x, 0, y, 0)` |
| `a == b` | Both same value | `(!a ∨ b) ∧ (a ∨ !b)` | `add_equal(x, y)` |
| `a != b` | Opposite values | `(a ∨ b) ∧ (!a ∨ !b)` | `add_neq(x, y)` |
| `a → b` | If a then b | `!a ∨ b` | `add_implication(id(x,1), id(y,1))` |
| `a ⊕ b` | XOR (exactly one) | `(a ∨ b) ∧ (!a ∨ !b)` | Same as `add_neq` |
...
**Problem:** `n` boolean variables, `m` clauses of form `(xi = a) OR (xj = b)`. Check satisfiability + output assignment.
...
**Why 2-SAT:** For door `i` with switches `p, q`, final state = `s_p XOR s_q`. Target `t_i` gives:
- If `t_i = 0` → `s_p == s_q` (equality)
- If `t_i = 1` → `s_p != s_q` (inequality)
...
* **Undirected cycle detection:** Forgetting to check `to != parent` when seeing visited node.
...
* **2-SAT clauses:** For `(a ∨ b)`, adding only one implication instead of both.
...
* **2-SAT** = each variable has 2 states + pair clauses → implication graph + SCC.
* All of this runs in **`O(n + m)`** and handles typical \(2 \cdot 10^5\) CF / AtCoder graphs.
