...
function getArticleDetail(pathname: string): BlogArticleNavigationDetail | null {
  ...
  if (!post) return null;
  return { href: `/blog/${post.slug}`, title: post.title, category: post.category, thumbnail: post.thumbnail };
}
    ...
    <>
      ...
      {transition && (
        <div className={`${styles.routeTransition} ${phaseClass}`} aria-hidden="true">
          <div className={styles.routeTransitionMedia}><Image src={transition.detail.thumbnail} alt="" fill sizes="min(72vw, 460px)" priority /></div>
