    ...
    <DiagramFrame label="The challenge" status="">
      ...
      <DiagramSvg width={900} height={250} ariaLabel="Recover the circuit from GDS, somehow find the right input, then extract the final output" className={styles.pipelineDesktop}>
        ...
        {stages.map(({ number, lines, color }, index) => {
          ...
          return <g key={number} transform={`translate(${x} 54)`}><rect width="236" height="142" rx="12" fill="#101010" fillOpacity="0.68" stroke={color} strokeOpacity="0.52" strokeWidth="1.35" /><circle cx="28" cy="28" r="14" fill={color} fillOpacity="0.1" stroke={color} strokeOpacity="0.72" /><text x="28" y="33" textAnchor="middle" fill={color} fontFamily="monospace" fontSize="12.5">{number}</text><text x="118" y={lines.length === 1 ? 78 : 70} textAnchor="middle" fill="#f5f5f5" fontFamily="monospace" fontSize="15" letterSpacing="0.5">{lines.map((line, lineIndex) => <tspan key={line} x="118" dy={lineIndex === 0 ? 0 : 22}>{line}</tspan>)}</text></g>;
        })}
      </DiagramSvg>
    </DiagramFrame>
  );
  ...
  return (
    <DiagramFrame label="Logical Schematic of an AND Gate" status="">
      ...
      <DiagramSvg className={styles.andGateMobile} width={360} height={380} ariaLabel="Inputs A and B enter a hardware AND gate and produce output Y only when both are one" contentScale={1} inset={12}>
        ...
        <g transform="translate(130 92)"><path d="M0 0h34c44 0 72 26 72 66s-28 66-72 66H0z" fill="#101010" stroke="#e8e8e8" strokeWidth="2" /><text x="47" y="72" textAnchor="middle" fill="#fff" fontFamily="monospace" fontSize="16">AND</text></g>
        <path d="M236 158H298" fill="none" stroke="#e8e8e8" strokeWidth="2.5" />
...
function SccDagDiagram({ progress }: ScrollDiagramProps): JSX.Element {
  ...
  return (
    <DiagramFrame label="State dependency" status="">
      ...
      <DiagramSvg className={styles.stateDagDesktop} width={760} height={570} ariaLabel="A four-register dependency loop condenses into one node, followed by the simplified state dependency DAG" contentScale={1} progress={progress} progressLabel={dagReveal > 0.82 ? 'Register groups form the dependency DAG' : collapse > 0.72 ? 'Collapse the dependency loop into one SCC' : 'Trace register-to-register dependencies'} showBoard={false}>
      ...
      <DiagramSvg className={styles.stateDagMobile} width={360} height={620} ariaLabel="A phone-friendly view of the simplified state dependency DAG" contentScale={1} inset={12} progress={progress} progressLabel="Register groups form the dependency DAG" showBoard={false}>
    ...
    <DiagramFrame label="SAT intuition" status="">
      <div className={styles.satSequence} aria-label="Walk through the four AND-gate truth-table assignments. The first three produce zero and do not satisfy the required target. A equals one and B equals one produce success equals one.">
        <div className={styles.satSchematic} data-output={activeRow.output}>
          ...
          <svg className={styles.satInputWires} viewBox="0 0 40 100" preserveAspectRatio="none" aria-hidden="true"><path d="M0 25H40M0 75H40" /></svg>
          <div className={styles.satAndGate}><svg viewBox="0 0 120 92" aria-hidden="true"><path d="M0 8h50c42 0 70 18 70 38S92 84 50 84H0z" /><text x="55" y="52" textAnchor="middle">AND</text></svg></div>
        ...
        <DiagramProgress progress={progress} />
      </div>
    </DiagramFrame>
  );
}
...
function SatTimelineDiagram({ progress }: ScrollDiagramProps): JSX.Element {
  ...
  const successReveal = clamp((progress - 0.86) / 0.1);
  const witnessChosen = witnessProgress >= 0.5;
  const success = successReveal >= 0.5;
    ...
    <DiagramFrame label="Bounded reachability" status="">
      <DiagramSvg className={styles.satTimelineDesktop} width={920} height={500} ariaLabel="The implementation resets at cycle zero, unrolls the recovered transition relation forward across a 123-cycle bound, constrains success to be one at an eligible cycle, and asks Z3 for one globally consistent trace. A verified slice is then replayed forward one enabled input bit per clock; success is first observed at cycle 122." progress={progress} progressLabel={status} showBoard={false}>
        ...
        {VERIFIED_WITNESS_SLICE.map((bit, index) => {
          ...
          return <g key={bit.cycle} opacity={mix(0.5, 1, witnessProgress)}><rect x={x} y="84" width="64" height="66" rx="5" fill={fed ? '#fb4e7c' : '#111'} fillOpacity={fed ? active ? 0.22 : 0.09 : 1} stroke={active ? '#f0c557' : fed ? '#fb4e7c' : witnessChosen ? '#626262' : '#363636'} strokeWidth={active ? 2 : 1.2} /><text x={x + 32} y="104" textAnchor="middle" fill={fed ? '#d0d0d0' : '#898989'} fontFamily="monospace" fontSize="11.5">CYCLE {bit.cycle.toString().padStart(2, '0')}</text><text x={x + 32} y="136" textAnchor="middle" fill={fed ? '#fff' : witnessChosen ? '#bbb' : '#888'} fontFamily="monospace" fontSize="25" fontWeight="800">{witnessChosen ? bit.value : '?'}</text>{index < VERIFIED_WITNESS_SLICE.length - 1 && <path d={`M${x + 64} 117H${x + 78}`} stroke={fed ? '#fb4e7c' : '#444'} strokeWidth="1.4" />}</g>;
        })}
        ...
        <rect x="54" y="248" width="594" height="112" rx="8" fill="#15110f" stroke="#e9b94f" strokeOpacity="0.78" />
        <text x="351" y="278" textAnchor="middle" fill="#c3ad79" fontFamily="monospace" fontSize="13">{fedCount ? 'FORWARD REPLAY · ONE ENABLED CLOCK' : 'UNROLL FORWARD · ONE TRANSITION PER CYCLE'}</text>
        ...
        <text x="351" y="338" textAnchor="middle" fill="#aa9669" fontFamily="monospace" fontSize="12">{fedCount ? `replay I = ${activeBit.value} at cycle ${activeBit.cycle}` : targetProgress > 0.5 ? 'add the bounded target, then solve all cycles together' : 'reset S₀, then add each recovered transition'}</text>

        <path d="M648 304H702" stroke="#666" strokeWidth="1.7" markerEnd="url(#time-arrow)" />
        <rect x="704" y="268" width="162" height="72" rx="8" fill={success ? '#0d1511' : '#121212'} stroke={success ? '#609d78' : '#484848'} strokeWidth={success ? 1.35 : 1.2} />
      ...
      <DiagramSvg className={styles.satTimelineMobile} width={360} height={500} ariaLabel="The recovered transition relation is unrolled forward, success is constrained within the bound, and Z3 chooses one globally consistent trace. A verified slice of cycles eight through thirteen is then replayed one enabled input bit per clock; success is first observed at cycle 122." progress={progress} progressLabel={status} showBoard={false}>
        <defs><marker id="time-arrow-mobile" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0 0L8 3L0 6z" fill="#777" /></marker></defs>
        <rect x="18" y="30" width="324" height="112" rx="7" fill="#090909" stroke="#2e2e2e" />
        {VERIFIED_WITNESS_SLICE.map((bit, index) => {
          const x = 28 + index * 52;
          const fed = index < fedCount;
          const active = index === activeIndex && fedCount > 0 && !success;
          return <g key={bit.cycle} opacity={mix(0.5, 1, witnessProgress)}><rect x={x} y="54" width="42" height="62" rx="4" fill={fed ? '#fb4e7c' : '#111'} fillOpacity={fed ? active ? 0.22 : 0.09 : 1} stroke={active ? '#f0c557' : fed ? '#fb4e7c' : witnessChosen ? '#626262' : '#363636'} strokeWidth={active ? 2 : 1.1} /><text x={x + 21} y="74" textAnchor="middle" fill={fed ? '#d0d0d0' : '#999'} fontFamily="monospace" fontSize="10.5">C{bit.cycle.toString().padStart(2, '0')}</text><text x={x + 21} y="104" textAnchor="middle" fill={fed ? '#fff' : witnessChosen ? '#bbb' : '#888'} fontFamily="monospace" fontSize="21" fontWeight="800">{witnessChosen ? bit.value : '?'}</text></g>;
        })}
        ...
        <rect x="34" y="196" width="292" height="94" rx="7" fill="#15110f" stroke="#e9b94f" strokeOpacity="0.78" />
        <text x="180" y="222" textAnchor="middle" fill="#c3ad79" fontFamily="monospace" fontSize="11.5">{fedCount ? 'FORWARD REPLAY · ONE CLOCK' : 'UNROLL FORWARD · ONE TRANSITION'}</text>
        ...
        <text x="180" y="274" textAnchor="middle" fill="#bca775" fontFamily="monospace" fontSize="11.5">{fedCount ? `replay I = ${activeBit.value} at cycle ${activeBit.cycle}` : targetProgress > 0.5 ? 'constrain target · solve all cycles' : 'reset S₀ · add each transition'}</text>
        <path d="M180 290V316" stroke="#666" strokeWidth="1.5" markerEnd="url(#time-arrow-mobile)" />
        <rect x="80" y="324" width="200" height="66" rx="7" fill={success ? '#0d1511' : '#121212'} stroke={success ? '#609d78' : '#484848'} strokeWidth={success ? 1.35 : 1.2} />
  ...
  return (
    <DiagramFrame label="Recovered circuit I/O" status="">
      ...
      <DiagramSvg width={820} height={330} ariaLabel="Each clock supplies one input bit while the circuit reads its current state, computes outputs, and stores the next state" className={styles.ioDesktop}>
        ...
        <rect x="610" y="82" width="154" height="68" fill="#101010" stroke="#7ce5a8" /><circle cx="634" cy="116" r="8" fill="#7ce5a8" /><text x="656" y="112" fill="#999" fontFamily="monospace" fontSize="11.5">1-BIT DECISION</text><text x="656" y="133" fill="#fff" fontFamily="monospace" fontSize="13">success</text>
...
function resultBlockStyle(progress: number, index: number): React.CSSProperties {
  ...
  const scaled = clamp(progress / revealEnd) * ASIC_WITNESS_BLOCKS.length;
  const activeIndex = progress >= revealEnd ? ASIC_WITNESS_BLOCKS.length : Math.min(ASIC_WITNESS_BLOCKS.length - 1, Math.floor(scaled));
...
function ResultDecode({ progress }: ScrollDiagramProps): JSX.Element {
  const solutionReveal = clamp((progress - 0.86) / 0.1);
  return <DiagramFrame label="SOLUTION" status=""><section className={`${styles.resultFigure} ${surfaceStyles.coolVerificationSurface}`} aria-label="The verified 121-bit witness is revealed one 11-bit block at a time before producing the OCaml comment two stars"><div className={styles.resultBits}>{ASIC_WITNESS_BLOCKS.map((block, index) => <span key={`${block}-${index}`} style={resultBlockStyle(progress, index)}>{block}</span>)}</div><div className={styles.resultText} style={{ opacity: solutionReveal, transform: `translate3d(0, ${(1 - solutionReveal) * 16}px, 0)` }}><span>(*</span><strong>TWO STARS</strong><span>*)</span></div><DiagramProgress progress={progress} /></section></DiagramFrame>;
}
