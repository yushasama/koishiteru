...
export function DiagramProgress({ progress, label }: DiagramProgressProps): JSX.Element {
  return <div className={styles.diagramProgress} aria-hidden="true">{label && <strong>{label}</strong>}<span><i style={{ transform: `scaleX(${clamp(progress)})` }} /></span></div>;
}

export function DiagramSvg({ width, height, ariaLabel, children, className, boardBottomInset, contentScale = 0.94, inset = 14, progress, progressLabel, showBoard = true, showProgress = true }: DiagramSvgProps): JSX.Element {
  ...
  return <svg className={svgClassName} viewBox={`0 0 ${width} ${height}`} role="img" aria-label={ariaLabel}>{showBoard && <rect className={styles.diagramBoard} x={inset} y={inset} width={width - inset * 2} height={height - inset - bottomInset} />}<g transform={`translate(${offsetX} ${offsetY}) scale(${contentScale})`}>{children}</g>{progress !== undefined && showProgress && <g className={styles.diagramSvgProgress} aria-hidden="true">{progressLabel && <text x={progressX} y={progressY - 12}>{progressLabel}</text>}<rect x={progressX} y={progressY} width={progressWidth} height="3" /><rect className={styles.diagramSvgProgressFill} x={progressX} y={progressY} width={progressWidth * clamp(progress)} height="3" /></g>}</svg>;
}
